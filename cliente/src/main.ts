...
async function stats() {
    const name= ((await verifyToken(localStorage.getItem('token') as string).catch(err => {alert(err.message)}))).name;
...
loginButton.addEventListener("click", async ()=>{
    ...
    const token=await login(name,pass).catch(err => {alert(err.message)});
...
signUpButton.addEventListener("click", async ()=>{
    ...
    const token=await signup(name,pass).catch(err => {alert(err.message)});;
    localStorage.setItem("token",token.token);
    changingDiv();
});


createLobyButton.addEventListener("click", async ()=>{
    ...
    if (!alreadyInMatchCode.code){
        ...
        document.getElementById('directJoin')?.addEventListener('click', async ()=>{
